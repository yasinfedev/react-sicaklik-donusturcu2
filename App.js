...
export default function App() {
  ...
  useEffect(() => {
    // Sayfa yüklendiğinde, varsayılan bir değerle inputValue state'ini set et
    setInputValue(''); // Bu değeri istediğiniz başlangıç değeriyle değiştirebilirsiniz
  }, []); // Boş dependency array, useEffect'in yalnızca bir kere çalışmasını sağlar
  ...
  function handleChange(event) {
    ...
    if (regex.test(value)) {
      ...
      setInputValue(value);
      setSonuc(Number(value));
    } else {
      // Geçersiz değer, isteğinize göre başka bir işlem yapabilirsiniz
      alert('Lütfen sayısal bir değer giriniz.');
    }
  }

  function handleClickFahr(event) {
    if (inputValue == '') {
      alert('Lütfen sayısal bir değer giriniz.');
    } else {
  ...
  function handleClickKelvin(event) {
    if (inputValue == '') {
      alert('Lütfen sayısal bir değer giriniz.');
    } else {
      event.preventDefault();
  ...
  function girilenDeger() {
    document.getElementById('girilenDeger').innerHTML = inputValue;
  }
